...
void MapDrawer::DrawMapPoints() {
    ...
    // cout << "(MapDrawer) Draw MapPoints, size = " << vpMPs.size() << endl;
...
void MapDrawer::DrawMapLines() {
    ...
    // cout << "(MapDrawer) Draw MapLines, size = " << vpMLs.size() << endl;
    ...
    glBegin(GL_LINES);
    for (set<MapLine*>::iterator sit = spRefMLs.begin();
         sit != spRefMLs.end(); sit++) {
        if ((*sit)->isBad()) {
            continue;
        }
        string label = (*sit)->GetSemanticLabel();
        vector<float> color = yolov5::Yolov5Detector::planarColorMap[label];
