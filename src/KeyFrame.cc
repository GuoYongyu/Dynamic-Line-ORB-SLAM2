...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
        ...
        fx(F.fx), fy(F.fy), cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy), mWidth(F.mWidth), mHeight(mHeight),
        ...
        mvpMapLines(F.mvpMapLines), mvPlanarBoxes(F.mvPlanarBoxes), 
        mPlanarBoxExpansionRate(F.mPlanarBoxExpansionRate), mLineIntersectionRate(F.mLineIntersectionRate) {
    mnId = nNextId++;
...
/**
 * @brief Estimate the scene depth of the current key frame, q=2 represents the median
 * @param q q=2
 * @return Median Depth
 */
...
// Functions below are line-related
